...
def parse_profile(notices):
    for notice in reversed(notices):
        ...
        if match:
            ...
            for key in ("probes", "dimensions", "sort_bound", "candidates", "pages", "getitems_calls"):
                if key in values:
...
def run_group(conn, args, experiment, name, probes, raw_rows, summaries):
    ...
    with conn.cursor() as cur:
        ...
        measured = []
        for query_id, query in enumerate(queries[args.warmup:]):
...
def main():
    ...
    run.add_argument("--phase", choices=("a", "b", "2a"), required=True)
