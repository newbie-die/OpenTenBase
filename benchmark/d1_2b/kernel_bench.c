		  ...
		  int block_size, int baseline, int passes)
{
	...
	result.elapsed_seconds = seconds_now() - start;
	result.tsc_cycles = __rdtscp(&tsc_aux) - tsc_start;
	_mm_lfence();
	return result;
}
...
main(int argc, char **argv)
{
	...
	snprintf(timing_path, sizeof(timing_path), "%s/phase_d1_2b_timing_raw.csv", argv[2]);
	snprintf(correctness_path, sizeof(correctness_path), "%s/phase_d1_2b_correctness.csv", argv[2]);
	...
	fprintf(timing, "workload,probes,config,block_size,trial,passes,candidates_per_pass,total_candidates,elapsed_seconds,ns_per_candidate,tsc_cycles,tsc_cycles_per_candidate,sink\n");
	...
	for (int w = 0; w < 5; w++)
	{
		...
		for (int config = 0; config < 4; config++)
		{
			...
			char config_name[32];
			snprintf(config_name, sizeof(config_name), "%s", baseline ? "baseline" : block_size == 64 ? "progressive_b64" : block_size == 128 ? "progressive_b128" : "progressive_b256");
			...
			{
				double elapsed_so_far = seconds_now() - benchmark_start;
				double eta = completed_trials ? elapsed_so_far * (total_configs * trials - completed_trials) / completed_trials : 0.0;
				fprintf(stderr, "config=%d/%d current_kernel=%s workload=%s trial=%d/%d elapsed=%.1fs ETA=%.1fs\n",
						completed + 1, total_configs, config_name, workload->name,
						trial, trials, elapsed_so_far, eta);
				TrialResult result = run_trial(workload, metadata, candidates, queries,
										block_size, baseline, passes);
				uint64_t timed_candidates = (uint64_t) workload->count * passes;
				fprintf(timing, "%s,%d,%s,%d,%d,%d,%zu,%" PRIu64 ",%.9f,%.9f,%" PRIu64 ",%.9f,%.17g\n",
						workload->name, workload->probes, config_name, block_size,
						trial, passes, workload->count, timed_candidates, result.elapsed_seconds,
						result.elapsed_seconds * 1e9 / timed_candidates, result.tsc_cycles,
						(double) result.tsc_cycles / timed_candidates, timing_sink);
				fflush(timing);
				completed_trials++;
			}
			completed++;
