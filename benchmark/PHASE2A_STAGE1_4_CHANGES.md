...
Stage 1 已经是独立提交 `f9c4d329`。当前建议只提交 Stage 2–4 核心源码和本说明文档：
...
git add \
  ...
  contrib/pgvector/src/ivfflat.h \
  contrib/pgvector/src/ivfscan.c \
  benchmark/PHASE2A_STAGE1_4_CHANGES.md

git commit \
  -m "ivfflat: add limit-aware bounded sort fallback" \
  -m "Use executor-propagated logical bounds to derive conservative physical bounds and enable PostgreSQL bounded tuplesort for eligible non-iterative IVFFlat scans." \
  -m "Detect bounded candidate exhaustion before reading past the tuplesort bound, rebuild a full sort from the existing list pages, and skip TIDs already returned by the index AM so filtered LIMIT and OFFSET queries retain full-sort semantics." \
  -m "Add bounded and fallback profiling fields and document Phase 2A stages 1 through 4."
```

该命令不会重复提交 Stage 1 的 Executor/Index AM 文件，也不会把 `/workspace/benchmark` 独立目录中的运行结果或脚本误加入 OpenTenBase 仓库。
